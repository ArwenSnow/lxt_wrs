if __name__ == '__main__':
    ...
    import robot_sim.robots.gofa12_robot.gofa12_robot as gf
    ...
    base = wd.World(cam_pos=[1, 1, .5], lookat_pos=[0, 0, .2])  #创建了一个世界对象，指定摄像机位置和目光所指位置
    gm.gen_frame().attach_to(base) #生成一个坐标系，附加到世界对象“base”上

    robot_s = gf.GOFA12() #创建一个robot_s对象，引用了gofa12机器人模型
    robot_s.gen_meshmodel(rgba=(0,1,0,1)).attach_to(base) #调用了robot_s对象的gen_meshmodel()方法，生成机器人的几何模型，并附加到base上(初位）
