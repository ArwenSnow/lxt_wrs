...
def load_grasp_list(pair_type, debug_folder):  # 从debug_folder中加载抓取信息，根据pair_type筛选所需的抓取数据，以列表的形式返回所有抓取数据。
    """
    pair_type:指定抓取信息的类别
...
def load_single_grasp(pair_type, pair_id, rot_id, jaw_width_ratio):  # 从debug_folder加载抓取信息,并根据指定的参数，返回一个单一抓取数据
...
if __name__ == '__main__':
    ...
    obj.set_pos(np.array([0.6, 0, -0.014+0.038]))
    obj.attach_to(base)
    ...
    for approach_conf in approach_conf_list:
        ...
        if path_app is not None:
            ...
            if path_gri is not None:
                ...
                for item in path_app[::-1]:                    # 反向遍历全部路径
                    rbt_s.fk('arm', item)
