...
def calculate_pos(pos1, pos2):
    return np.linalg.norm(pos1 - pos2)


def calculate_rotmat(rotmat1, rotmat2):
    dot_product = np.trace(np.dot(rotmat1.T, rotmat2))
    angle = np.arccos((dot_product - 1) / 2)
    return np.abs(np.pi - angle)


def cost(grasp1, grasp2, w_pos=.5, w_rot=.5):
    ...
    pos_diff = calculate_pos(f_pos_1, f_pos_2)

    f_rotmat_1 = rotmat_1.dot(l_rotmat)
    f_rotmat_2 = rotmat_2.dot(r_rotmat)
    rot_diff = calculate_rotmat(f_rotmat_1, f_rotmat_2)
...
if __name__ == '__main__':
    ...
    for i, (a1, t1) in enumerate(approach_conf_1_list):
        ...
        edge_info[(tuple(a1), tuple(t1))] = (lftjcenter[i], None)
    ...
    for i, (_, t1) in enumerate(approach_conf_1_list):
        for j, (a2, _) in enumerate(approach_conf_2_list):
            ...
            edge_info[(tuple(t1), tuple(a2))] = (lftjcenter[i], rgtjcenter[j])

    for j, (a2, t2) in enumerate(approach_conf_2_list):
        graph[tuple(a2)][tuple(t2)] = 1
        edge_info[(tuple(a2), tuple(t2))] = (None, rgtjcenter[j])
