...
def update_robot_joints_2(robot_s, jnt_values):
    ...
    try:
        ...
        robot_s.fk(jnt_values=jnt_values)
        # 生成新的机器人模型
        new_mesh = robot_s.gen_meshmodel(toggle_tcpcs=True, rgba=[.7, .7, .7, .5])
        new_mesh.attach_to(base)
...
if __name__ == '__main__':
    ...
    path_11 = []
    pos_1, rot_1 = [], []
    pos_init, rot_init = make_circle_with_rot(np.array([.3, .3, .5]), np.eye(3))
    ...
    for p, r in zip(pos_1, rot_1):
        conf = robot_2.ik(component_name='arm', tgt_pos=p, tgt_rotmat=r)
        if conf is not None:
            path_2.append(conf)
    ...
    # robot_1.fk('arm', path_1[0])
    # robot_2.fk('arm', path_2[0])
    ...
    # robot_mesh_1.attach_to(base)
    # robot_mesh_2.attach_to(base)
    # base.run()
