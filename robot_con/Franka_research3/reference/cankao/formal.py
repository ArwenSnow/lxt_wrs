...
NATAPP_HOST = 'b946f478.natappfree.cc'
NATAPP_PORT = 32795
try:
...
current_robot_mesh = robot_1.gen_meshmodel()
...
def update_task(task):
    ...
    xme3p_conf = update_robot_joints_2(new_p_2, new_r_2)
    prev_angles = angles_display.copy()

    try:
        msg_bytes = struct.pack('7f', *xme3p_conf.astype(np.float32))  # 转 float32 发送
        sender.sendto(msg_bytes, server_addr)
        time.sleep(0.001)
        print("发送关节角度到c++端:", xme3p_conf)
        print("\n")
    except Exception as e:
        print(f"发送失败: {e}")
