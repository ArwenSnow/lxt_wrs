        ...
        while 1:
            ...
            if label.LABEL_UPDATE:
                if len(label.LABEL_POINTS_ALL):
                    # 更新静态标注图
                    if label.LABEL_POINTS[0] == label.LABEL_POINTS[2] and label.LABEL_POINTS[1] == label.LABEL_POINTS[
                        3]:  # 标注的是球形
            ...
            if label.LABELING_POINTS or label.LABELING_POINTS_WIDTH or label.LABEL_SHOW_ANGLE_CIRCLE or label.LABEL_SHOW_GRASP_LINE:
                ...
                if label.LABEL_POINTS[0] == label.LABEL_POINTS[2] and label.LABEL_POINTS[1] == label.LABEL_POINTS[
                    ...
                    if label.LABEL_SHOW_ANGLE_CIRCLE:
                        cv2.circle(png_visual_dynamic, (label.LABEL_POINTS[0], label.LABEL_POINTS[1]), 20,
                                   label.COLOR_RED, 1)
                    ...
                    if label.LABEL_SHOW_ANGLE_CIRCLE:
                        ...
                        cv2.circle(png_visual_dynamic,
                                   (int(label.LINE_SIDE_POINTS[0][0]), int(label.LINE_SIDE_POINTS[0][1])), 5,
                                   label.COLOR_GREEN, -1)  # 注意颜色
                        cv2.circle(png_visual_dynamic,
                                   (int(label.LINE_SIDE_POINTS[1][0]), int(label.LINE_SIDE_POINTS[1][1])), 5,
                                   label.COLOR_RED, -1)  # 注意颜色
                ...
                if label.LABEL_SHOW_GRASP_LINE:
                    ...
                    if label.LABEL_SHOW_GRASP_LINE in [label.GRASP_RED, label.GRASP_GREEN]:
                        ...
                        if angle < math.pi:
                            cv2.line(png_visual_dynamic, (cx, cy), (int(cx + dx), int(cy - dy)), label.COLOR_LIGHT_BLUE,
                                     1)
                        else:
                            cv2.line(png_visual_dynamic, (cx, cy), (int(cx - dx), int(cy + dy)), label.COLOR_LIGHT_BLUE,
                                     1)
