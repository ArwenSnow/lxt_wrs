...
class GraspDataset(torch.utils.data.Dataset):
    def __init__(self, file_path, start=0.0, end=1.0, ds_rotate=0, output_size=360, include_depth=True,
                 include_rgb=False, argument=False):
        """
        :param output_size: int 输入网络的图像尺寸
        :param include_depth: 网络输入是否包括深度图
        :param include_rgb: 网络输入是否包括 RGB图
        """
...
if __name__ == '__main__':
    dataset_path = r'E:\ABB-Project\3D_Visual_code\label_grasp\test'
    # 加载训练集
    ...
    for x, y in train_data:
        ...
        ang_img = (torch.atan2(y[2], y[1]) / 2.0).cpu().numpy().squeeze()  # 弧度

        grasps = []
        rows, cols = np.where(pos_img == 1)  # 抓取点
        ...
        cv2.imwrite(r'E:\ABB-Project\3D_Visual_code\ggcnn\grasp_data/' + str(count) + '_region.png', im_grasp_region)
        cv2.imwrite(r'E:\ABB-Project\3D_Visual_code\ggcnn\grasp_data/' + str(count) + '_line.png', im_grasp_line)
