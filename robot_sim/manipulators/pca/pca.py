...
class Pca(mi.ManipulatorInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), homeconf=np.zeros(7), name='pca', enable_cc=True):
        ...
        self.jlc.jnts[2]['loc_rotmat'] = rm.rotmat_from_axangle([1, 0, 0], -math.pi/2)
        self.jlc.jnts[2]['loc_motionax'] = np.array([0, 0, -1])

        self.jlc.jnts[3]['loc_pos'] = np.array([-101/1000, 0, 0])
        self.jlc.jnts[3]['loc_rotmat'] = rm.rotmat_from_axangle([0, 1, 0], math.pi/2)
        self.jlc.jnts[3]['loc_motionax'] = np.array([0, 0, -1])

        self.jlc.jnts[4]['loc_pos'] = np.array([0, 0, -154/1000])
        self.jlc.jnts[4]['loc_rotmat'] = rm.rotmat_from_axangle([0, 1, 0], -math.pi/2)
        self.jlc.jnts[4]['loc_motionax'] = np.array([0, 0, -1])

        self.jlc.jnts[5]['loc_pos'] = np.array([-47.65/1000, 0, 0])
        x_4 = rm.rotmat_from_axangle([1, 0, 0], math.pi)
        self.jlc.jnts[5]['loc_rotmat'] = x_4@rm.rotmat_from_axangle([0, 1, 0], math.pi/2)
        self.jlc.jnts[5]['loc_motionax'] = np.array([0, 0, -1])

        self.jlc.jnts[6]['loc_pos'] = np.array([-93.75/1000, -54.4/1000, -251.85/1000])
        self.jlc.jnts[6]['loc_rotmat'] = rm.rotmat_from_axangle([1, 0, 0], -math.pi/2)
        self.jlc.jnts[6]['loc_motionax'] = np.array([0, 0, 1])

        self.jlc.jnts[7]['loc_pos'] = np.array([65/1000, 0, 78/1000])
        self.jlc.jnts[7]['loc_rotmat'] = rm.rotmat_from_axangle([0, 1, 0], -math.pi/2)
...
if __name__ == '__main__':
    ...
    manipulator_meshmodel = Pca_manipulator.gen_meshmodel(toggle_jntscs=True)
