...
class ManipulatorInterface(object):
    ...
    def gen_urdf(self):
        ...
        WORLD_LNK = Link(name=parent, )
        group = []
        for i in range(self.jlc.ndof + 1):
            if i == 0:
                jnt = Joint(parent + "_" + robot_name + "_joint",
                            Origin(xyz="0 0 0", rpy="0 0 0"),
                            Parent(link=parent),
                            Child(link=robot_name + "_link_0"), type="fixed")
            else:
                jnt = _joint(i,
                             robot_name,
                             origin=[*self.jnts[i]['loc_pos'], *rotmat_to_euler(self.jnts[i]['loc_rotmat'])],
                             limit=self.jnts[i]['motion_rng'],
                             type=self.jnts[i]['type'])
            group.append(jnt)
            lnk = _link(i,
                        robot_name,
                        origin=[*self.lnks[i]['loc_pos'], *rotmat_to_euler(self.lnks[i]['loc_rotmat'])],
                        # x, y, z, r, p, y
                        mass=self.lnks[i]['mass'],
                        geom_origin=self.lnks[i]['com'].tolist())
            group.append(lnk)
        URDF = Robot(WORLD_LNK,
                     *group,
                     name=self.name, )
        return str(URDF)
