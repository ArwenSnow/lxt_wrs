...
import robot_sim.end_effectors.gripper.xc330gripper1.xc330gripper1_old.xc330gripper1 as xc1
import robot_sim.end_effectors.gripper.xc330gripper2.xc330gripper2_old.xc330gripper2 as xc2
import robot_sim.end_effectors.gripper.PGC_300_60_W_S.PGC_300_60_W_S_old.PGC as PGC
...
class reconfgripper(gp.GripperInterface):
    ...
    def lft_center_pos_main(self, m_jawwidth):
        lft_center_pos_local = np.array([0.00236, 0, 0.056735])  # 小夹爪中心点在自身坐标系中的位置
        lft_origin_pos_main = np.array([-0.0276 - m_jawwidth / 2, 0, 0.136])  # 小夹爪零点在大夹爪坐标系中的位置
        m_rotmat = np.eye(3)
    ...
    def rgt_center_pos_main(self, m_jawwidth):
        ...
        rgt_origin_pos_global = np.array([0.0276 + m_jawwidth / 2, 0, 0.136])  # 小夹爪零点在大夹爪坐标系中的位置
        m_rotmat = np.eye(3)
    ...
    def center_pos_global(self, contact_center_pos, contact_center_rotmat, mg_jawwidth, g="l"):
        if g == "l":
            x = contact_center_pos + contact_center_rotmat.dot([-0.05048 - mg_jawwidth, 0, 0])
            y = contact_center_pos + contact_center_rotmat.dot([-0.02524 - mg_jawwidth / 2, 0, -0.192735])
