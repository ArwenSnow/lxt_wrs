...
class Reconfgripper(gp.GripperInterface):
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), cdmesh_type='box', name='maingripper',
                 enable_cc=True):
    ...
    def enable_cc(self, toggle_cdprimit):
        if toggle_cdprimit:
            super().enable_cc()
            # cdprimit
            self.cc.add_cdlnks(self.body.lft, [0, 1])
            self.cc.add_cdlnks(self.body.rgt, [1])
            self.cc.add_cdlnks(self.lft.lft, [0, 1])
            self.cc.add_cdlnks(self.lft.rgt, [1])
            self.cc.add_cdlnks(self.rgt.lft, [0, 1])
            self.cc.add_cdlnks(self.rgt.rgt, [1])
            activelist = [self.body.lft.lnks[0],
                          self.body.lft.lnks[1],
                          self.body.rgt.lnks[1],
                          self.lft.lft.lnks[0],
                          self.lft.lft.lnks[1],
                          self.lft.rgt.lnks[1],
                          self.rgt.lft.lnks[0],
    ...
    def fix_to(self, pos, rotmat):
        ...
        self.lft.fix_to(pos=self.body.lft.jnts[2]['gl_posq'], rotmat=self.body.lft.jnts[2]['gl_rotmatq'])
        self.rgt.fix_to(pos=self.body.rgt.jnts[2]['gl_posq'], rotmat=self.body.rgt.jnts[2]['gl_rotmatq'])
...
if __name__ == '__main__':
    ...
    grpr.fix_to(pos=np.array([.3, .0, .0]), rotmat=np.eye(3))
    grpr.gen_meshmodel().attach_to(base)
    grpr.show_cdprimit()
    base.run()
