...
class frank_research3(gp.GripperInterface):

    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), coupling_offset_pos=np.zeros(3),
                 coupling_offset_rotmat=np.eye(3), cdmesh_type='convex_hull', name='frank_research3',
                 enable_cc=True):
        ...
        self.lft.lnks[0]['mesh_file'] = cm.CollisionModel(os.path.join(this_dir, "meshes", "hand_base.stl"),
        ...
        self.lft.jnts[1]['loc_pos'] = np.array([0, -0.00158, 0.1059])
        self.lft.jnts[1]['loc_rotmat'] = rm.rotmat_from_euler(0, 0, 0)
        self.lft.jnts[1]['type'] = 'prismatic'
        self.lft.jnts[1]['motion_rng'] = [0, .03925]
        self.lft.jnts[1]['loc_motionax'] = np.array([0, -1, 0])
        self.lft.lnks[1]['name'] = "finger_left"
        self.lft.lnks[1]['mesh_file'] = cm.CollisionModel(os.path.join(this_dir, "meshes", "left.stl"),
                                                          cdprimit_type="user_defined",
        ...
        self.rgt.jnts[1]['loc_pos'] = np.array([0, 0.00158, 0.1059])
        ...
        self.rgt.lnks[1]['name'] = "finger_right"

        self.rgt.lnks[1]['mesh_file'] = cm.CollisionModel(os.path.join(this_dir, "meshes", "right.stl"),
                                                          cdprimit_type="user_defined",
        ...
        # jaw width
        self.jawwidth_rng = [0.0, .0785]
        # jaw center
        self.jaw_center_pos = np.array([0, 0, .1059]) + coupling_offset_pos
        # collision detection
        # self.all_cdelements = []
        # self.enable_cc(toggle_cdprimit=enable_cc)
    ...
    def _finger_right_cdnp(name, radius):
        ...
        collision_primitive_c0 = CollisionBox(Point3(0, 0.00531, 0),
                                              x=.00884 + radius, y=0.00761 + radius, z=.00884 + radius)
    ...
    def _finger_left_cdnp(name, radius):
        collision_node = CollisionNode(name)
        collision_primitive_c0 = CollisionBox(Point3(0, -0.00531, 0),
                                              x=.00888 + radius, y=0.00761 + radius, z=.00884 + radius)
        collision_node.addSolid(collision_primitive_c0)
    ...
    def _hnd_base_cdnp(name, radius):
        collision_node = CollisionNode(name)
        collision_primitive_c0 = CollisionBox(Point3(0, 0, .031245),
                                              x=.032 + radius, y=0.105 + radius, z=.03725 + radius)
...
if __name__ == '__main__':
    ...
    gm.gen_frame(pos=[0,0,0.1059]).attach_to(base)
    # cm.CollisionModel("meshes/dual_realsense.stl", expand_radius=.001).attach_to(base)
    grpr = frank_research3(enable_cc=True)
    # grpr.show_cdmesh()
    if grpr.is_mesh_collided():
        print("mesh collide")
    else:
        print("mesh not collide")
    # grpr.open()
    grpr.show_cdprimit()    # 展示碰撞体
    # grpr.jaw_to(0.0385)
    grpr.gen_meshmodel().attach_to(base)    # 放到环境中
