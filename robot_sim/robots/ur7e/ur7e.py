...
class UR7E(ri.RobotInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name="gofa5", enable_cc=True):
        ...
        self.arm = rbt.UR5E(pos=pos + [0.4, 0.1, 0.5],
                             rotmat=rotmat,
        ...
        # self.machine = jl.JLChain(pos=pos,
        #                           rotmat=rotmat,
        #                           homeconf=np.zeros(0),
        #                           name='machine')
        #
        # self.machine.lnks[0]['collision_model'] = gm.GeometricModel(
        #     os.path.join(this_dir, "meshes", "3D_2020.6.25(2).stl"))
        # self.machine.lnks[0]['rgba'] = [.35, .35, .35, 1]
        # self.machine.reinitialize()
