...
class XME3P(ri.RobotInterface):

    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name="xme3p", enable_cc=True):
        super().__init__(pos=pos, rotmat=rotmat, name=name)
        this_dir, this_filename = os.path.split(__file__)

        # arm
        # self.arm_homeconf = np.array([0., 0., 0, math.pi/2, 0., 0., 0])
        # self.arm_homeconf = np.array([0.2, 0.35, 0.2, 2.0, -0.25, 0.7, 1.5])
        self.arm_homeconf = np.array([0, 1.0035, 0, 1.0035, 0, 1.0035, 0])
        self.arm = rbt.XME3P(pos=pos,
                             rotmat=rotmat,
        ...
        # self.base = jl.JLChain(pos=pos + [0.4, 0.1, 0.5],
        #                           rotmat=rotmat,
        #                           homeconf=np.zeros(0),
        #                           name='machine')
        #
...
if __name__ == '__main__':
    ...
    # conf = np.array([0, 0, 0, 0, 0, 0, 0])
    # robot_s.fk('arm', conf)
    # robot_s.gen_meshmodel().attach_to(base)

    # robot_s.show_cdprimit()
    # test_pos = np.array([.25, .3, .15])
    # test_pos_list = [np.array([0.3, 0.5, 0.2])]
    ...
    #
    #     conf = robot_s.ik('arm', test_pos, test_rot, seed_jnt_values=robot_s.get_jnt_init("arm"))
    #
