...
import robot_sim.end_effectors.gripper.xc330gripper.xc330gripper as hnd
...
class bigGripper(ri.RobotInterface):

    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name="bigGripper", enable_cc=True):
        super().__init__(pos=pos, rotmat=rotmat, name=name)
        this_dir, this_filename = os.path.split(__file__)
        # arm
        self.arm = rbt.PGC(pos=pos,
                            rotmat=rotmat,
                            homeconf=np.zeros(6),
                            name='arm', enable_cc=False)

        # lft gripper
        self.hnd = hnd.Gripper(pos=self.arm.jnts[-1 ]['gl_posq'],
                                   rotmat=self.arm.jnts[-1 ]['gl_rotmatq'],
                            name='hnd_s', enable_cc=False)

        # rgt gripper
        self.rgt = hnd.Gripper(pos=self.arm.jnts[-1]['gl_posq']+np.array([-.1252, 0, 0]),
                               rotmat=np.dot(rm.rotmat_from_axangle([0, 0, 1], math.pi * 1), self.arm.jnts[-1 ]['gl_rotmatq']),
                               name='hnd_s', enable_cc=False)
...
if __name__ == '__main__':
    ...
    robot_s = bigGripper(enable_cc=True)
    robot_s.jaw_to(.1)   #执行机器人夹爪的动作，将夹爪移动到指定的位置（0.02）
