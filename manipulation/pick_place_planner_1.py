...
import motion.optimization_based.incremental_nik_1 as inik
import motion.probabilistic.rrt_connect as rrtc
import manipulation.approach_depart_planner_1 as adp
...
class PickPlacePlanner(adp.ADPlanner):
    ...
    def gen_holding_moveto(self,
        ...
        jawwidth_bk = self.robot_s.get_jawwidth(hnd_name)
        # final
        conf_list = []
        jawwidthlist = []
        objpose_list = []
        ...
        first_conf = self.robot_s.ik(component_name,
        ...
        self.robot_s.fk(component_name=component_name, jnt_values=first_conf)
        ...
        for i in range(len(obj_pose_list) - 1):
            ...
            if approach_distance is None:
                approach_distance = 0
            # depart linear
            conf_list_depart = self.inik_slvr.gen_rel_linear_motion(component_name=component_name,
                                                                    hnd_name=hnd_name,
            ...
            objpose_list_depart = self.gen_object_motion(component_name=component_name,
            ...
            if use_rrt:  # if use rrt, we shall find start and goal conf first and then perform rrt
                ...
                conf_list_approach = self.inik_slvr.gen_rel_linear_motion(component_name=component_name,
                                                                          hnd_name=hnd_name,
                ...
                if conf_list_approach is None:
                    ...
                    self.robot_s.fk(component_name=component_name, jnt_values=jnt_values_bk)
                    return None, None, None
                conf_list_middle = self.rrtc_planner.plan(component_name=component_name,
                ...
                seed_conf = conf_list_depart[-1]
                self.robot_s.fk(component_name=component_name, jnt_values=seed_conf)
                mid_start_tcp_pos, mid_start_tcp_rotmat = self.robot_s.get_gl_tcp(hnd_name)
                mid_goal_tcp_pos = goal_jaw_center_pos - approach_direction * approach_distance
                mid_goal_tcp_rotmat = goal_jaw_center_rotmat
                conf_list_middle = self.inik_slvr.gen_linear_motion(component_name=component_name,
            ...
            objpose_list_approach = self.gen_object_motion(component_name=component_name,
                                                           conf_list=conf_list_approach,
                                                           obj_pos=rel_obj_pos,
                                                           obj_rotmat=rel_obj_rotmat,
                                                           type='relative')
            jawwidthlist_middle = self.gen_jawwidth_motion(conf_list_middle, jaw_width)
            objpose_list_middle = self.gen_object_motion(component_name=component_name,
    ...
    def gen_pick_and_place_motion(self,
        ...
        if use_incremental:
            ...
            common_grasp_id_list, _ = self.find_common_graspids(component_name,
                                                                hnd_name,
                                                                grasp_info_list,
                                                                goal_homomat_list)
            print(common_grasp_id_list)
            print('//')
        ...
        for grasp_id in common_grasp_id_list:
            ...
            conf_list_middle, jawwidthlist_middle, objpose_list_middle = \
                self.gen_holding_moveto(component_name=component_name,
                                        hnd_name=hnd_name,
                                        objcm=objcm,
            ...
            conf_list_depart, jawwidthlist_depart = \
                self.gen_depart_motion(component_name=component_name,
                                       hnd_name=hnd_name,
